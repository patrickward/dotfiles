...
`dnsmasq` is a lightweight DNS server. `../topics/homebrew/setup.sh` does three things:
...
For Caddy site snippets and day-to-day commands, see [`caddy.md`](caddy.md).
...
- **PostgreSQL**: Use [Postgres.app](https://postgresapp.com) for multi-version support.
  `../topics/homebrew/setup.sh` writes `/etc/paths.d/postgresapp` so `psql`, `pg_dump`, etc. resolve. Two
